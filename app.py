...
# Custom CSS for Dark Fintech Glassmorphism Aesthetic
st.markdown("""
<style>
  ...
  .metric-card.accent-cyan { border-left-color: #06b6d4; }
...
st.caption("Banking & Financial Security Assurance Engine | OWASP ASVS-L3 & FAPI Compliant")

st.markdown("`🔒 FAPI 1.0/2.0` | `💳 PCI-DSS v4.0` | `🛡️ OWASP ASVS-L3` | `🔑 PSD2 RTS`")
st.divider()
...
with c1:
    st.markdown('<div class="metric-card">🏆 <b>TARGET STANDARD</b><br><h3>ASVS Level 3</h3><small>Highest Financial Rating</small></div>', unsafe_allow_html=True)
with c2:
    st.markdown(f'<div class="metric-card accent-cyan">📋 <b>TOTAL CONTROLS</b><br><h3>{total_controls} Controls</h3><small>Across Core Banking Domains</small></div>', unsafe_allow_html=True)
with c3:
    st.markdown('<div class="metric-card accent-amber">🛡️ <b>THREAT PROTECTION</b><br><h3>Anti-Replay / SCA</h3><small>Mitigating Tampering & ATO</small></div>', unsafe_allow_html=True)
with c4:
    st.markdown(f'<div class="metric-card accent-indigo">📊 <b>AUDIT ENGINE</b><br><h3 style="color:{grade_color};">{score_pct}%</h3><small style="color:{grade_color};">{grade_text}</small></div>', unsafe_allow_html=True)
...
with tab_scanner:
    ...
    target_url = st.text_input(
        "Target URL to Scan:",
        value="https://banksec-core-4q43jtsstksw55gvvtzcml.streamlit.app/",
        placeholder="https://your-banking-api.com"
    )
    
    if st.button("🚀 Launch Real-Time Scan"):
        if not target_url:
            st.error("Please enter a valid target URL.")
        else:
            with st.spinner("Executing real-time HTTP/HTTPS security probe..."):
                try:
                    ...
                    if not is_https:
                        findings.append({
                            ...
                            "severity": "CRITICAL",
                            "control": "API-01",
                            "desc": "Target communicates over unencrypted HTTP. Credentials can be intercepted.",
                    ...
                    if 'X-Frame-Options' in headers or 'x-frame-options' in headers:
                        ...
                        findings.append({
                            ...
                            "severity": "HIGH",
                            "control": "AUTH-03",
                            "desc": "UI can be embedded in attacker iFrames to steal user clicks.",
                    ...
                    server_banner = headers.get('Server', headers.get('server', '')) or headers.get('X-Powered-By', headers.get('x-powered-by', ''))
                    if server_banner:
                        findings.append({
                    ...
                    st.session_state['audit_state']['API-01'] = 'PASS' if is_https and header_score > 0 else 'FAIL'
                    st.session_state['audit_state']['API-02'] = 'PASS' if header_score >= 3 else 'FAIL'
                    st.session_state['audit_state']['AUTH-01'] = 'PASS' if ('Content-Security-Policy' in headers or 'content-security-policy' in headers) else 'FAIL'
                    st.session_state['audit_state']['AUTH-03'] = 'PASS' if ('X-Frame-Options' in headers or 'x-frame-options' in headers) else 'FAIL'
                    ...
                    st.subheader("Automated Security Findings")
                    if not findings:
                        st.success("🎉 Excellent! No critical HTTP security header or protocol vulnerabilities discovered on this endpoint.")
                    else:
                        for f in findings:
                            with st.expander(f"🔴 [{f['severity']}] {f['title']} (Mapped to Control: {f['control']})"):
                                st.write(f"**Description:** {f['desc']}")
                                st.info(f"**Remediation:** {f['recommendation']}")
                                
                    st.toast("🎉 Interactive Audit Updated! View findings in the 'Read-Only Interactive Audit' tab.", icon="✅")
                    
                except Exception as e:
                    st.error(f"❌ Real-time scan failed: {str(e)}")

# TAB 2: Security Framework
with tab_framework:
    st.subheader("Banking Security Controls & Production Code Remediation")
    st.write("Browse production-ready security control implementations for Express Node.js, Spring Boot Java, and Python.")
    ...
    for c in BANKING_CONTROLS:
        with st.expander(f"[{c['id']}] {c['name']} — [{c['severity']}]"):
            ...
            st.markdown(f"`{c['asvs']}` | `{c['pci']}` | `{c['fapi']}`")
            st.caption(f"Code Blueprint: {c['code_title']}")
...
with tab_audit:
    ...
    report_data = {
        ...
        "tool": "BankSec Core Streamlit Assurance Engine",
